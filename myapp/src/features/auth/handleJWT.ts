...
const expirationalKey = 'token-expiration';

export function saveToken(authData: authenticationResponse) {
    localStorage.setItem(tokenKey, authData.token);
    localStorage.setItem(expirationalKey, authData.expiration.toString());
}
...
export function getClaims() : claim[] {
    ...
    const expiration = localStorage.getItem(expirationalKey)!;
...
export function logout() {
    ...
    localStorage.removeItem(expirationalKey);
}
